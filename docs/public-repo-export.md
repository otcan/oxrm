...
4. Run `pnpm install --frozen-lockfile`, `pnpm typecheck`, and `pnpm build`.
5. Start the demo instance and run the demo smoke path from [instances.md](instances.md).
6. Commit the sanitized tree as the first public commit.
7. Tag the first public release only after governance files and release docs are present.
